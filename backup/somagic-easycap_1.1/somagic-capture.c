						...
						if (vs->active_line_count > (lines_per_field - 8)) {
							if (vs->field == 0) {
								if (frames_generated < frame_count || frame_count == -1) {
									/*
										throw away the first 2 frames, which are usually frames
										where the active video started mid-frame.
										this ensures requesting a single frame to use
										as a screenshot will be a good frame
									*/
									if(throw_away_frame > 2)
									{
